/*
    ...
    *******
    Special modified version for use with I-CubeX microDigs in standalone (midi mapped) mode

    This allows USB and Wireless digitizers to send MIDI messages directly to a MIDI port
    which can be used by any MIDI application. Requires SiLabs VCP driver (should be built in
    for most modern distros), and tested to work with Raspberry Pi as well 

    https://github.com/I-CubeX/microDig-LinuxDriver 

    Feb 2017, johnty@infusionsystems.com

    Modifications:  
	- added flag "SND_SEQ_PORT_TYPE_MIDI_GENERIC" so it can be seen by rtmidi
	- changed name to microDig-standalone
    *******
...
void arg_set_defaults(arguments_t *arguments)
{
	...
	char *name_tmp		= (char *)"microDig-port";
